...
# make new file with new ammonia San Diego data
# and clean up names
# 7/2023
##########################
import glob 
...
for d_i in range(len(csv_files)):    
    ...
    if 'CDOM Voltage (V) WET' in df_new.keys():
        df_new.rename(index=str,columns={'CDOM Voltage (V) WET':'CDOM_voltage'},inplace=True)
    ...
    if 'CDOM Voltage WET (V)' in df_new.keys():
        df_new.rename(index=str,columns={'CDOM Voltage WET (V)':'CDOM_voltage'},inplace=True)
    ...
    if 'Chl-a USC/UCLA [~$m~#g/L]' in df_new.keys(): 
        ...
        # multiply by 1000 to turn mg/L to ug/L
        df_new['Chl-a_USC_UCLA'] = df_new['Chl-a_USC_UCLA'].apply(lambda x: x*(1000))
    ...
    if 'Chlorophyll-a Voltage (V) WET' in df_new.keys():
        df_new.rename(index=str,columns={'Chlorophyll-a Voltage (V) WET':'Chl-a_voltage'},inplace=True)
    ...
    if 'Chlorophyll-a Voltage WET (V)' in df_new.keys():
        df_new.rename(index=str,columns={'Chlorophyll-a Voltage WET (V)':'Chl-a_voltage'},inplace=True)
...
objs_space = ['salinity','CDOM','descent_rate','specific_density','oxygen_saturation_percent','dissolved_oxygen']
...
########################
print('appending San Diego data')
...
df_sd.rename(index=str,columns={'Chlorophyll_a_Voltage_V_WET':'Chl-a_voltage'},inplace=True)
...
# append San Diego ammonia data
df_nh4 = pd.read_csv('Ammonia_2010-2017.csv',low_memory=False)
# parse dates
df_nh4['SampleDate'] = pd.to_datetime(df_nh4['SampleDate'],format='%d-%b-%y')

# dates
nh4_datelist = list(df_nh4['SampleDate'])
nh4_datenum = date2num(nh4_datelist,dt_units)

df_nh4.rename(index=str,columns={'SampleDate':'date'},inplace=True)
df_nh4['date'] = np.copy(nh4_datenum)

# variables
df_nh4.rename(index=str,columns={'Depth_Meter':'depth'},inplace=True)
df_nh4.rename(index=str,columns={'Value':'ammonia-N'},inplace=True)

# locations
nh4_lat = np.empty((len(df_nh4['Station'])))
nh4_lon = np.empty((len(df_nh4['Station'])))
for n_i in range(len(df_nh4['Station'])):
    print(str(n_i)+' of '+str(len(df_nh4['Station'])))
    nh4_lat[n_i] = sd_stn['latitude'][np.where(df_nh4['Station'][n_i]==sd_stn['station'])[0][0]]
    nh4_lon[n_i] = sd_stn['longitude'][np.where(df_nh4['Station'][n_i]==sd_stn['station'])[0][0]]

df_nh4['latitude'] = nh4_lat
df_nh4['longitude'] = nh4_lon

df_nh4 = df_nh4.drop(['SampleNumber','Station','Parameter','Qualifier','Units'],axis=1)

df = df.append(df_nh4,ignore_index=True)

# append San Diego "kelp" data (nearshore sites)
df_kelp = pd.read_csv('CSD_CTDData_PLOO_Kelp_1991-2023.csv',low_memory=False)
# parse dates
df_kelp['sample_date'] = pd.to_datetime(df_kelp['sample_date'],format='%Y-%m-%d')

# dates
kelp_datelist = list(df_kelp['sample_date'])
kelp_datenum = date2num(kelp_datelist,dt_units)

df_kelp.rename(index=str,columns={'sample_date':'date'},inplace=True)
df_kelp['date'] = np.copy(kelp_datenum)

# variables

df_kelp.rename(index=str,columns={'depth_meter':'depth'},inplace=True)
df_kelp.rename(index=str,columns={'temp':'temperature'},inplace=True)
df_kelp.rename(index=str,columns={'density':'specific_density'},inplace=True)
df_kelp.rename(index=str,columns={'ph':'pH'},inplace=True)
df_kelp.rename(index=str,columns={'dox':'dissolved_oxygen'},inplace=True)
df_kelp.rename(index=str,columns={'pct_light':'light_transmission'},inplace=True)
df_kelp.rename(index=str,columns={'cdom':'CDOM'},inplace=True)

# locations
kelp_lat = np.empty((len(df_kelp['station'])))
kelp_lon = np.empty((len(df_kelp['station'])))
for n_i in range(len(df_kelp['station'])):
    print(str(n_i)+' of '+str(len(df_kelp['station'])))
    kelp_lat[n_i] = sd_stn['latitude'][np.where(df_kelp['station'][n_i]==sd_stn['station'])[0][0]]
    kelp_lon[n_i] = sd_stn['longitude'][np.where(df_kelp['station'][n_i]==sd_stn['station'])[0][0]]

df_kelp['latitude'] = kelp_lat
df_kelp['longitude'] = kelp_lon

df_kelp = df_kelp.drop(['station','fluorometry'],axis=1)

df = df.append(df_kelp,ignore_index=True)

# San Diego South Bay Ocean Outfall (SBOO) 
# sampling outside of quarterly monitoring
df_sboo_out = pd.read_csv('CSD_CTDData_SBOO_1995-2023.csv',low_memory=False)
# parse dates
df_sboo_out['sample_date'] = pd.to_datetime(df_sboo_out['sample_date'],format='%Y-%m-%d')

# dates
sboo_out_datelist = list(df_sboo_out['sample_date'])
sboo_out_datenum = date2num(sboo_out_datelist,dt_units)

df_sboo_out.rename(index=str,columns={'sample_date':'date'},inplace=True)
df_sboo_out['date'] = np.copy(sboo_out_datenum)

# variables

df_sboo_out.rename(index=str,columns={'depth_meter':'depth'},inplace=True)
df_sboo_out.rename(index=str,columns={'temp':'temperature'},inplace=True)
df_sboo_out.rename(index=str,columns={'density':'specific_density'},inplace=True)
df_sboo_out.rename(index=str,columns={'ph':'pH'},inplace=True)
df_sboo_out.rename(index=str,columns={'dox':'dissolved_oxygen'},inplace=True)
df_sboo_out.rename(index=str,columns={'pct_light':'light_transmission'},inplace=True)
df_sboo_out.rename(index=str,columns={'cdom':'CDOM'},inplace=True)

# locations
sboo_out_lat = np.empty((len(df_sboo_out['station'])))
sboo_out_lon = np.empty((len(df_sboo_out['station'])))
for n_i in range(len(df_sboo_out['station'])):
    print(str(n_i)+' of '+str(len(df_sboo_out['station'])))
    sboo_out_lat[n_i] = sd_stn['latitude'][np.where(df_sboo_out['station'][n_i]==sd_stn['station'])[0][0]]
    sboo_out_lon[n_i] = sd_stn['longitude'][np.where(df_sboo_out['station'][n_i]==sd_stn['station'])[0][0]]

df_sboo_out['latitude'] = sboo_out_lat
df_sboo_out['longitude'] = sboo_out_lon

df_sboo_out = df_sboo_out.drop(['station','fluorometry'],axis=1)

df = df.append(df_sboo_out,ignore_index=True)

# San Diego PLOO sampling outside of quarterly monitoring
# 1991-2003
df_ploo_out1 = pd.read_csv('CSD_CTDData_PLOO_Offshore_1991-2003.csv',low_memory=False)
# parse dates
df_ploo_out1['sample_date'] = pd.to_datetime(df_ploo_out1['sample_date'],format='%Y-%m-%d')

# dates
ploo_out1_datelist = list(df_ploo_out1['sample_date'])
ploo_out1_datenum = date2num(ploo_out1_datelist,dt_units)

df_ploo_out1.rename(index=str,columns={'sample_date':'date'},inplace=True)
df_ploo_out1['date'] = np.copy(ploo_out1_datenum)

# variables

df_ploo_out1.rename(index=str,columns={'depth_meter':'depth'},inplace=True)
df_ploo_out1.rename(index=str,columns={'temp':'temperature'},inplace=True)
df_ploo_out1.rename(index=str,columns={'density':'specific_density'},inplace=True)
df_ploo_out1.rename(index=str,columns={'ph':'pH'},inplace=True)
df_ploo_out1.rename(index=str,columns={'dox':'dissolved_oxygen'},inplace=True)
df_ploo_out1.rename(index=str,columns={'pct_light':'light_transmission'},inplace=True)
df_ploo_out1.rename(index=str,columns={'cdom':'CDOM'},inplace=True)

# locations
ploo_out1_lat = np.empty((len(df_ploo_out1['station'])))
ploo_out1_lon = np.empty((len(df_ploo_out1['station'])))
for n_i in range(len(df_ploo_out1['station'])):
    print(str(n_i)+' of '+str(len(df_ploo_out1['station'])))
    ploo_out1_lat[n_i] = sd_stn['latitude'][np.where(df_ploo_out1['station'][n_i]==sd_stn['station'])[0][0]]
    ploo_out1_lon[n_i] = sd_stn['longitude'][np.where(df_ploo_out1['station'][n_i]==sd_stn['station'])[0][0]]

df_ploo_out1['latitude'] = ploo_out1_lat
df_ploo_out1['longitude'] = ploo_out1_lon

df_ploo_out1 = df_ploo_out1.drop(['station','fluorometry'],axis=1)

# take only dates since 1996
df = df.append(df_ploo_out1[df_ploo_out1.date>0],ignore_index=True)

# PLOO outside of quarterly sampling 
# 2003-2023
df_ploo_out2 = pd.read_csv('CSD_CTDData_PLOO_Offshore_2003-2023.csv',low_memory=False)
# parse dates
df_ploo_out2['sample_date'] = pd.to_datetime(df_ploo_out2['sample_date'],format='%Y-%m-%d')

# dates
ploo_out2_datelist = list(df_ploo_out2['sample_date'])
ploo_out2_datenum = date2num(ploo_out2_datelist,dt_units)

df_ploo_out2.rename(index=str,columns={'sample_date':'date'},inplace=True)
df_ploo_out2['date'] = np.copy(ploo_out2_datenum)

# variables

df_ploo_out2.rename(index=str,columns={'depth_meter':'depth'},inplace=True)
df_ploo_out2.rename(index=str,columns={'temp':'temperature'},inplace=True)
df_ploo_out2.rename(index=str,columns={'density':'specific_density'},inplace=True)
df_ploo_out2.rename(index=str,columns={'ph':'pH'},inplace=True)
df_ploo_out2.rename(index=str,columns={'dox':'dissolved_oxygen'},inplace=True)
df_ploo_out2.rename(index=str,columns={'pct_light':'light_transmission'},inplace=True)
df_ploo_out2.rename(index=str,columns={'cdom':'CDOM'},inplace=True)

# locations
ploo_out2_lat = np.empty((len(df_ploo_out2['station'])))
ploo_out2_lon = np.empty((len(df_ploo_out2['station'])))
for n_i in range(len(df_ploo_out2['station'])):
    print(str(n_i)+' of '+str(len(df_ploo_out2['station'])))
    ploo_out2_lat[n_i] = sd_stn['latitude'][np.where(df_ploo_out2['station'][n_i]==sd_stn['station'])[0][0]]
    ploo_out2_lon[n_i] = sd_stn['longitude'][np.where(df_ploo_out2['station'][n_i]==sd_stn['station'])[0][0]]

df_ploo_out2['latitude'] = ploo_out2_lat
df_ploo_out2['longitude'] = ploo_out2_lon

df_ploo_out2 = df_ploo_out2.drop(['station','fluorometry'],axis=1)

# take only dates before 2019
df = df.append(df_ploo_out2[df_ploo_out2.date<8500],ignore_index=True)
...
savename_clean = 'central_bight_master_database_1998_2019_1D_validation_2023.nc'
