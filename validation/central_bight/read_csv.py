...
csv_files_long = sorted(glob.glob('/data/project1/minnaho/validation/C*.csv'))

# get only file names
csv_files = [ f[f.index('n/')+2:] for f in csv_files_long]
'''
# create lists to append data to, put in netCDF after
# all data from all years appended
time_list = []
depths_list = 
...
for data in csv_files:
    df = pd.read_csv(data,sep=',',parse_dates=True,encoding='ISO-8859-1')

    # parse dates (put them in right format/datetime format)
    df['Date'] = pd.to_datetime(df['Date'],format='%m/%d/%Y')

    # convert pandas Timestamp to datetime objects
    dates_dt = [ df['Date'][i].to_pydatetime() for i in range(len(df['Date']))] 

    dates_nc = date2num(dates_dt,'days since 1998-07-08')

    # find lat/lon by referencing lat_lon_csv
    for loc in df['Station ID']:
    ...
    # append data
    time_list.append(dates_nc)
    depths_list.append(df.['Depth(m)']
'''
...
for data in csv_files:    
    ...
    if 'Ammonia-N (ug/L)' in df_new.keys():
        ...
        # multiply by 1000 to turn ug/L to mg/L
        df_new['ammonia-N'] = df_new['ammonia-N'].apply(lambda x: x*1000)
...
#df = df.drop(['Unnamed: 25','Unnamed: 26','Unnamed: 27','Unnamed: 28','Unnamed: 29','Qual_EC','Qual_ENT','Qual_FC','Qual_NH3','Qual_TC','Void','Ammonia QA Batch'],axis=1)

df = df.drop(['Qual_EC','Qual_ENT','Qual_FC','Qual_NH3','Qual_TC','Void','Ammonia QA Batch','Unnamed: 25','Unnamed: 26','Unnamed: 27','Unnamed: 28','Unnamed: 29'],axis=1)
...
dates_nc = date2num(date_list,'days since 1998-07-08')
