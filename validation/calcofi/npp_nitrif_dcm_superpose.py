...
parallels = np.arange(0,90,.25)
meridians = np.arange(180,360,.25)
...
for n_i in range(len(num_vars_c)):
    fig1,axes1 = plt.subplots(1,1,figsize=[w_plot,h_plot])
    fig2,axes2 = plt.subplots(1,1,figsize=[w_plot,h_plot])
    # observation data
    ...
    map_ax1 = Basemap(projection='stere',resolution='h',lat_0=lat_mean,lon_0=lon_mean,llcrnrlat=lat_min,urcrnrlat=lat_max,llcrnrlon=lon_min,urcrnrlon=lon_max,ax=axes1) 
    x,y = map_ax1(lon_c,lat_c)
    npp_plot_o = map_ax1.scatter(x,y,c=npp_obs,cmap=npp_cmap,marker='o',edgecolor=c_edge,s=m_size)
    ...
    map_ax2 = Basemap(projection='stere',resolution='h',lat_0=lat_mean,lon_0=lon_mean,llcrnrlat=lat_min,urcrnrlat=lat_max,llcrnrlon=lon_min,urcrnrlon=lon_max,ax=axes2) 
    x,y = map_ax2(lon_r,lat_r)
    nit_plot_o = map_ax2.scatter(x,y,c=nit_obs,cmap=nit_cmap,marker='o',edgecolor=c_edge,s=m_size)
    ...
    x,y = map_ax1(lon_nc,lat_nc)
    # set colorbar max and min of model the same as the observation
    if npp_plot_o.get_clim()[0]!=None:
        vmin_npp_o = npp_plot_o.get_clim()[0]
        vmax_npp_o = npp_plot_o.get_clim()[1]
        npp_plot_m = map_ax1.pcolor(x,y,npp_flat,cmap=npp_cmap,vmin=vmin_npp_o,vmax=vmax_npp_o)
    else: 
        npp_plot_m = map_ax1.pcolor(x,y,npp_flat,cmap=npp_cmap)
    ...
    x,y = map_ax2(lon_nc,lat_nc)
    if nit_plot_o.get_clim()[0]!=None:
        vmin_nit_o = nit_plot_o.get_clim()[0]
        vmax_nit_o = nit_plot_o.get_clim()[1]
        nit_plot_m = map_ax2.pcolor(x,y,nit_flat,cmap=nit_cmap,vmin=vmin_nit_o,vmax=vmax_nit_o)
    else: 
        nit_plot_m = map_ax2.pcolor(x,y,nit_flat,cmap=nit_cmap)

    # replot observation on top of model map
    x,y = map_ax1(lon_c,lat_c)
    npp_plot_o = map_ax1.scatter(x,y,c=npp_obs,cmap=npp_cmap,marker='o',edgecolor=c_edge,s=m_size)
    x,y = map_ax2(lon_r,lat_r)
    nit_plot_o = map_ax2.scatter(x,y,c=nit_obs,cmap=nit_cmap,marker='o',edgecolor=c_edge,s=m_size)
    # colorbar
    p1 = axes1.get_position().get_points().flatten()
    cb_ax1 = fig1.add_axes([p1[2]+0.02,p1[1],0.01,p1[3]-p1[1]])
    cb1 = fig1.colorbar(npp_plot_m,cax=cb_ax1,orientation='vertical',format='%.0e')
    ...
    p2 = axes2.get_position().get_points().flatten()
    cb_ax2 = fig2.add_axes([p2[2]+0.02,p2[1],0.01,p2[3]-p2[1]])
    cb2 = fig2.colorbar(nit_plot_m,cax=cb_ax2,orientation='vertical',format='%.0e')
    ...
    axes2.text(70553,109411,season,fontsize=22)
    
#    savename1 = fig_path+'npp_'+season+'_dcm_alltime'
#    savename1 = fig_path+'npp_'+season+'_dcm_alltime_zoom_edgewhite'
#    savename1 = fig_path+'npp_'+season+'_dcm_19992000_zoom'
#    savename1 = fig_path+'npp_'+season+'_dcm_19992000_superpose'
#    savename1 = fig_path+'npp_'+season+'_dcm_alltime_superpose'
#    savename2 = fig_path+'nit_'+season+'_dcm_alltime_haline'
#    savename2 = fig_path+'nit_'+season+'_dcm_alltime_zoom_haline'
    savename2 = fig_path+'nit_'+season+'_dcm_alltime_zoom_haline_new'
#    savename2 = fig_path+'nit_'+season+'_dcm_alltime_edgewhite'
