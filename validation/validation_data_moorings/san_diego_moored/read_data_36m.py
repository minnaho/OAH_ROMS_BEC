...
lat_unique = [32.54215833]
lon_unique = [-117.1991867]
...
for t_i in range(len(year)):
    ...
    time_num = date2num(d_t,'minutes since 2014-01-01-00-03')
...
# vertical array is shorter than others (missing data in last excel sheet)
# so put this array into an array of the right shape with other values as nan
vert_m_array = np.empty((len(year),9))
vert_m_array.fill(np.nan)

vert_m_array[:len(vert_arr),:] = vert_arr

####################
# make netCDF file
####################

adcp_data = Dataset('SD_ADCP_36m.nc','w')

adcp_data.title = 'San Diego 36 meter ADCP data from Wendy Enright from San Diego Public Utilities Department'

adcp_data.description = '2014-01-01 to 2017-12-31, 4 m bins, 5 min frequency'

# dimensions
time_dim = adcp_data.createDimension('time',len(year))
depth_dim = adcp_data.createDimension('depth',9)
...
time_var.units = 'minutes since 2014-01-01 00:03'
