...
out_path = './roms_output_avg/l2_scb_avg_'
    ...
    for m_i in range(s_m,e_m):
        ...
        if m_i not in months_w_31_days:
            ...
            if m_i == 2 and y_i not in leap_years:
                ndays = 28
        for d_i in range(1,ndays+1):
            fi_dt = 'Y'+str(y_i)+'M'+'%02d'%m_i+'D'+'%02d'%d_i
            #subprocess.call('zslice '+str(dep0)+' '+str(dep1)+' --vars=u,v '+grid_path+' '+out_path+fi_dt+'.nc',shell=True)
            subprocess.call('zslice '+str(dep1)+' --vars=u,v '+grid_path+' '+out_path+fi_dt+'.nc',shell=True)
