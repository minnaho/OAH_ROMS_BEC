...
%% 05/2018              				        %%
%% ============================================================ %%
disp(['2D total production program starts .... on:  ',  datestr(now)])
...
%% find the find one by one
 ...
 file_avg = [rep_avg,'/',repavg_avg(fr,1).name];
...
%% read the variables
   ...
   dataout  = ncread(file, 'TOT_PROD') ;
   dataout = permute(dataout, [3 2 1]);
   OM = squeeze(dataout(:,:,:)) ;
   OM(z>DD1 & z<DD2)=NaN; OM = squeeze(nanmean(OM,1)) ;
...
disp(['2D total production program ends .... on:  ',  datestr(now)])
