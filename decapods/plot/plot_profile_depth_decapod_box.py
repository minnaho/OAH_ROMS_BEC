...
# SF/Monterey Bay box, channel islands
ij_pt = [
        [[990,620],[1140,710]],
        [[265,600],[575,650]]]
ij_nm = ['SF Bay coast','Channel Island']
#ij1_pt = [[990,620],[1140,710]] 
#ij2_pt = [[265,650],[575,600]] 
...
for v_i in range(len(exp_variables)):
    ...
    for f_i in range(len(files_nc)):
        ...
        data_var = np.array(data_nc.variables[exp_variables[v_i]])
        data_var[data_var==0] = np.nan
        for p_i in range(len(ij_pt)):
            # convert to % time
            if exp_variables[v_i]=='Duration':
                pt_value = ((np.nanmean(data_var[ij_pt[p_i][0][1]:ij_pt[p_i][1][1],ij_pt[p_i][0][0]:ij_pt[p_i][1][0]]))/num_days_full)*100
            else:
                pt_value = np.nanmean(data_var[ij_pt[p_i][0][1]:ij_pt[p_i][1][1],ij_pt[p_i][0][0]:ij_pt[p_i][1][0]])
...
plt.savefig(save_path+'profile_decapods_juvenile_mort_box.png',bbox_inches='tight')
