...
nc_data1 = Dataset(exp_files[1],'r')
nc_data2 = Dataset(exp_files[0],'r')
fig,axes = plt.subplots(2,3,figsize=[fig_w,fig_h],sharex=True,sharey=True)
for var in range(len(exp_variables)):
    if 'jun_sep' in exp_files[0]:
        num_days_select = num_days_jun_sep
    if 'mar_may' in exp_files[0]:
        num_days_select = num_days_mar_may
    if 'jun_nov' in exp_files[0]:
        num_days_select = num_days_jun_nov
    if 'upwell' in exp_files[0] and 'echinoderm' in exp_files[0]:
        num_days_select = num_days_apr_jul
    if 'upwell' in exp_files[0] and 'mort' in exp_files[0] and 'decapod' in exp_files[0]:
        num_days_select = num_days_mar_jul
    if 'upwell' in exp_files[0] and 'diss' in exp_files[0] and 'decapod' in exp_files[0]:
    ...
    # pteropods
    if (species == 'pteropod') or ('bottom' in exp_files[0] and 'deep' not in exp_files[0]):
        variable_data = np.array(nc_data1.variables[exp_variables[var]][:,:])[y0:yE,x0:xE]*mask_grid
    # echinoderms
    else:
        variable_data_tr = np.transpose(nc_data1.variables[exp_variables[var]][:,:])
        variable_data = variable_data_tr[y0:yE,x0:xE]*mask_grid
    # make land white by setting values to nan
    if species=='echinoderm' and 'bottom' in exp_files[0]: 
...
for var in range(len(exp_variables)):
    map_ax = Basemap(projection='stere',resolution='h',lat_0=lat_mean,lon_0=lon_mean,llcrnrlat=lat_min,urcrnrlat=lat_max,llcrnrlon=lon_min,urcrnrlon=lon_max,ax=axes.flat[var+3])
    x,y = map_ax(lon_grid,lat_grid)
    # pteropods
    if (species == 'pteropod') or ('bottom' in exp_files[0] and 'deep' not in exp_files[0]):
        variable_data = np.array(nc_data2.variables[exp_variables[var]][:,:])[y0:yE,x0:xE]*mask_grid
    # echinoderms
    else:
        variable_data_tr = np.transpose(nc_data2.variables[exp_variables[var]][:,:])
        variable_data = variable_data_tr[y0:yE,x0:xE]*mask_grid
    # make land white by setting values to nan
    if species=='echinoderm' and 'bottom' in exp_files[0]: 
        variable_data = variable_data*h_echino
    variable_data[variable_data==0] = np.nan
    axes.flat[var+3].set_title(exp_variables[var],fontsize=subplot_title_font) 
    h_plt = map_ax.contour(x,y,h_grid,baths,colors='k',linewidths=1)
    plt.clabel(h_plt,fontsize=9,fmt='%1i')
    map_ax.drawstates()
    map_ax.drawcountries()
    ...
    #p = map_ax.pcolor(x,y,variable_data,cmap=cmap_plot,rasterized=True)
    p = map_ax.pcolor(x,y,variable_data,cmap=cmap_plot)
    p0 = axes.flat[var+3].get_position().get_points().flatten()
    cb_ax = fig.add_axes([p0[2]+0.01,p0[1],cb_w,p0[3]-p0[1]])
    cb_im = fig.colorbar(p,cax=cb_ax,orientation='vertical')
    cb_im.set_label(cb_label,fontsize=axis_font)
    cb_im.ax.tick_params(axis='both',which='major',direction='in',labelsize=axis_tick_size)
    cb_im.ax.tick_params(axis='both',which='minor',direction='in',labelsize=axis_tick_size)
axes.flat[0].text(729141,1.03455E6,'(a)',fontsize=axis_tick_size+2)
axes.flat[1].text(729141,1.03455E6,'(b)',fontsize=axis_tick_size+2)
axes.flat[2].text(729141,1.03455E6,'(c)',fontsize=axis_tick_size+2)
axes.flat[3].text(729141,1.03455E6,'(d)',fontsize=axis_tick_size+2)
axes.flat[4].text(729141,1.03455E6,'(e)',fontsize=axis_tick_size+2)
axes.flat[5].text(729141,1.03455E6,'(f)',fontsize=axis_tick_size+2)
fig.savefig(save_figs_path+'decapods_juvenile_mort_adult_survival_fornina.png',bbox_inches='tight')
im = Image.open(save_figs_path+'decapods_juvenile_mort_adult_survival_fornina.png')
im = im.convert('RGB')
im.save(save_figs_path+'decapods_juvenile_mort_adult_survival_fornina.jpg')
