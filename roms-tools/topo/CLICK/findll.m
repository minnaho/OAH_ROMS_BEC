function  [i,j,mask] = findll(xr,yr,x,y,x0,y0,i,j,h,mask,sc0,sc1);
...
%   disp('inside')
      ...
      nmask = 0*mask;
      nmask(~mask) = -1e5;
      hold on;pcolor(x(j:j+1,i:i+1),y(j:j+1,i:i+1),h(j+1:j+2,i+1:i+2)+nmask(j+1:j+2,i+1:i+2));shading flat;caxis([sc0 sc1]);hold off
    else
%     disp('outside')
    end
   return
