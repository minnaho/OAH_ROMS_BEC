 function var = get_frc_IRONs(grd,BGCparam,n,t,bgctime)
 ...
 %%% Compute or retrieve interpolation coefs through triangulation
  fcoef_iron = [BGCparam.coefs_dir '/s2r_IronHuang_' BGCparam.name_vars{n} '.mat'] ;
  dummy_mask = squeeze(vari(end,:,:)) ; dummy_mask(dummy_mask~=0)=1 ;
  if exist(fcoef_iron,'file')
     ...
     save(fcoef_iron,'elem2d','coef2d','nnel','A','-v7.3')
     toc
  end
