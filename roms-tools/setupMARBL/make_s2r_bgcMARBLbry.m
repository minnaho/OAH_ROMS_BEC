...
bgcname = '/data/project8/pdamien/ROMS_outputs/PACMED25KMmarbl/FORCINGS/pacmed25_bgcMARBLbry.' ;
...
BGCname1_vars = {'NO3' 'PO4' 'SiO3' 'O2' 'Fe' 'DIC' 'Alk' 'CHL'} ;
bgc_file = '/data/project8/pdamien/ROMS_outputs/PACMED25KMnew/BGC/pacmed_0p25_bgc_*.nc' ;
% CASE 2 : vars derived from other vars 
BGCname2_vars = {'spC' 'spP' 'spChl' 'spFe' 'spCaCO3' 'diatC' 'diatP' 'diatChl' 'diatFe' 'diatSi' 'diazC' 'diazP' 'diazChl' 'diazFe' 'zooC' 'DIC_ALT_CO2' 'ALK_ALT_CO2' 'Lig'} ;
BGCname2_coef = [3.375  0.01  0.675 1.35e-05  0.0675   0.2025   0.01    0.0675   1.35e-06  0.0675   0.0375  0.01   0.0075  7.5e-07   1.35     1             1           0.002] ;
BGCname2_vref = {'CHL'  'CHL' 'CHL'   'CHL'   'CHL'    'CHL'   'CHL'    'CHL'    'CHL'    'CHL'   'CHL'    'CHL'    'CHL'   'CHL'   'CHL'   'DIC'         'ALK'          'Fe'} ;
%
%---------------------------------------------------------------------------------------
...
for yy=start_year:end_year
   ...
   if obcflag(1)==1  %%   Southern boundary
      ...
      if strcmp(BGCname1_vars{v},'Alk')==1
      nccreate(file_bgcbry,'ALK_south','Dimensions',{'xi_rho',nx,'s_rho',nz,'time',0},'datatype','single');
      ncwrite(file_bgcbry,'ALK_south',var_int) ;
      else
   ...
   if obcflag(2)==1  %%   Eastern boundary
      ...
      if strcmp(BGCname1_vars{v},'Alk')==1
      nccreate(file_bgcbry,'ALK_east','Dimensions',{'eta_rho',ny,'s_rho',nz,'time',0},'datatype','single');
      ncwrite(file_bgcbry,'ALK_east',var_int) ;
      else
   ...
   if obcflag(3)==1  %%   Northern boundary
      ...
      if strcmp(BGCname1_vars{v},'Alk')==1
      nccreate(file_bgcbry,'ALK_north','Dimensions',{'xi_rho',nx,'s_rho',nz,'time',0},'datatype','single');
      ncwrite(file_bgcbry,'ALK_north',var_int) ;
      else
   ...
   if obcflag(4)==1  %%   Western boundary
      ...
      if strcmp(BGCname1_vars{v},'Alk')==1
      nccreate(file_bgcbry,'ALK_west','Dimensions',{'eta_rho',ny,'s_rho',nz,'time',0},'datatype','single');
      ncwrite(file_bgcbry,'ALK_west',var_int) ;
      else
