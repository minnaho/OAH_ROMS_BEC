function [lon1,lat1] = inverse_move_and_turn(psid,thetad,alphad,lambdad, ...
                                                                lon,lat)
...
disp([ 'inverse_move_and_turn :: psi=', num2str(psid), ...
...
for j=1:neta
  for i=1:nxi
    csT=cos(lat(i,j));  xr=csT*cos(lon(i,j)-psi0);  % apply longitudinal
     zr=sin(lat(i,j));  yr=csT*sin(lon(i,j)-psi0);  % shift first

    x = a11*xr +a21*yr +a31*zr;  % Note that matrix a_ij is transposed
    y = a12*xr +a22*yr +a32*zr;  % here relative to its counterpart in
    z = a13*xr +a23*yr +a33*zr;  % move_and_turn.m
