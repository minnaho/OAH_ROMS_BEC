...
%---------------------------------------------------------------------------------------
%
    romsdir     = '/data/project8/pdamien/ROMS_outputs/PACMED12KMnew/';
    chdgrd      = [romsdir 'FORCINGS/pacmed12_grd.nc'];
    chdbry_list = dir([romsdir 'RUNS/Y2019/nepac_bry.2019*.nc']);
%
    obcflag=[0 0 1 0];    % open boundaries flag (1=open , [S E N W])
%
    bgc_tra = {'PO4' 'NO3' 'SiO3' 'NH4' 'Fe' 'O2' 'DIC' 'Alk' 'DOC' 'DON' 'DOFE' 'DOP' 'DONR' 'DOPR' ...
               'ZOOC' 'SPC' 'SPCHL' 'SPFE' 'SPCACO3' 'DIATC' 'DIATCHL' 'DIATFE' 'DIATSI' 'DIAZC' 'DIAZCHL' ...
               'DIAZFE' 'NO2' 'N2' 'N2O'} ;
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

for K=1:length(chdbry_list)

    chdbry = [chdbry_list(K).folder '/' chdbry_list(K).name] ; 
...
%    mask = zeta(:,1) ; mask(mask>1e20)=NaN ; 
    ...
    for trc=1:length(bgc_tra)
    trac = ncread(chdbry,[bgc_tra{trc} '_south']) ;
    for i=1:length(IndNaN)
        trac(IndNaN(i),:,:) = trac(IndReplace(i),:,:);
    end
    ncwrite(chdbry,[bgc_tra{trc} '_south'],trac);
    end
...
%    mask = zeta(:,1) ; mask(mask>1e20)=NaN ;
    ...
    for trc=1:length(bgc_tra)
    trac = ncread(chdbry,[bgc_tra{trc} '_east']) ;
    for i=1:length(IndNaN)
        trac(IndNaN(i),:,:) = trac(IndReplace(i),:,:);
    end
    ncwrite(chdbry,[bgc_tra{trc} '_east'],trac);
    end
...
%    mask = zeta(:,1) ; mask(mask>1e20)=NaN ;
    ...
    for trc=1:length(bgc_tra)
    trac = ncread(chdbry,[bgc_tra{trc} '_north']) ;
    for i=1:length(IndNaN)
        trac(IndNaN(i),:,:) = trac(IndReplace(i),:,:);
    end
    ncwrite(chdbry,[bgc_tra{trc} '_north'],trac);
    end
...
%    mask = zeta(:,1) ; mask(mask>1e20)=NaN ;
    ...
    for trc=1:length(bgc_tra)
    trac = ncread(chdbry,[bgc_tra{trc} '_west']) ;
    for i=1:length(IndNaN)
        trac(IndNaN(i),:,:) = trac(IndReplace(i),:,:);
    end
    ncwrite(chdbry,[bgc_tra{trc} '_west'],trac);
    end
