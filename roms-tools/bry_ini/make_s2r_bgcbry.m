...
end_date   = datenum(2016,12,31);
%
grdname = '/data/project7/pdamien/ROMS_outputs/ATLMED12KM/FORCINGS/atlmed_grd.nc' ;  
%
bryname = '/data/project7/pdamien/ROMS_outputs/ATLMED12KM/FORCINGS/atlmed_bry_' ; 
...
bgc_file = '/data/project7/pdamien/ROMS_outputs/ATLMED12KM/BGC/atlmed_bgc_*.nc' ;  
...
for yy=start_year:end_year
   ...
   if obcflag(1)==1  %%   Southern boundary
      ...
      for z=nz-1:-1:1
          ...
          for t=1:size(dens_bry,3)
              testvec = squeeze(dens_bry(:,z,t)) ;
              dens_bry(:,z,t) = fillmissing(testvec,'nearest') ;
          end
      end
      for v=1:length(BGCname1_vars)
      var_int = dens_bry.*0 ;
      for t=1:length(bry_day)
          rho_t = coeft(t,3).*dens_bry_south(:,:,coeft(t,2)) + (1-coeft(t,3)).*dens_bry_south(:,:,coeft(t,1)) ; 
          for z=nz-1:-1:1
          rho_t(:,z) = rho_t(:,z) + z*0.0000001 ;
          end 
          var_t = coeft(t,3).*vars_bry_south(:,:,coeft(t,2),v) + (1-coeft(t,3)).*vars_bry_south(:,:,coeft(t,1),v) ;
          var_interp = var_t.*NaN ; 
          for i=1:size(rho_t,1)
%              var_int(i,:,t) = interp1(squeeze(rho_t(i,:)),squeeze(var_t(i,:)), ...
%                                       squeeze(dens_bry(i,:,t)),'nearest','extrap');
              testvec = interp1(squeeze(rho_t(i,:)),squeeze(var_t(i,:)),squeeze(dens_bry(i,:,t)));
              var_interp(i,:) = fillmissing(testvec,'nearest') ;
          end
          var_int(:,:,t) = fillmissing(var_interp,'nearest') ; 
      end
   ...
   if obcflag(2)==1  %%   Eastern boundary
      ...
      var_int = dens_bry.*0 ;
      for t=1:length(bry_day)
          rho_t = coeft(t,3).*dens_bry_east(:,:,coeft(t,2)) + (1-coeft(t,3)).*dens_bry_east(:,:,coeft(t,1)) ;
          for z=nz-1:-1:1
          rho_t(:,z) = rho_t(:,z) + z*0.0000001 ;
          end
          var_t = coeft(t,3).*vars_bry_east(:,:,coeft(t,2),v) + (1-coeft(t,3)).*vars_bry_east(:,:,coeft(t,1),v) ;
          var_interp = var_t.*NaN ;
          for i=1:size(rho_t,1)
%              var_int(i,:,t) = interp1(squeeze(rho_t(i,:)),squeeze(var_t(i,:)), ...
%                                       squeeze(dens_bry(i,:,t)),'nearest','extrap');
   ...
   if obcflag(3)==1  %%   Northern boundary
      ...
      var_int = dens_bry.*0 ;
      for t=1:length(bry_day)
          rho_t = coeft(t,3).*dens_bry_north(:,:,coeft(t,2)) + (1-coeft(t,3)).*dens_bry_north(:,:,coeft(t,1)) ;
          for z=nz-1:-1:1
          rho_t(:,z) = rho_t(:,z) + z*0.0000001 ;
          end
          var_t = coeft(t,3).*vars_bry_north(:,:,coeft(t,2),v) + (1-coeft(t,3)).*vars_bry_north(:,:,coeft(t,1),v) ;
          var_interp = var_t.*NaN ;
          for i=1:size(rho_t,1)
%              var_int(i,:,t) = interp1(squeeze(rho_t(i,:)),squeeze(var_t(i,:)), ...
%                                       squeeze(dens_bry(i,:,t)),'nearest','extrap');
   ...
   if obcflag(4)==1  %%   Western boundary
      ...
      var_int = dens_bry.*0 ;
      for t=1:length(bry_day)
          rho_t = coeft(t,3).*dens_bry_west(:,:,coeft(t,2)) + (1-coeft(t,3)).*dens_bry_west(:,:,coeft(t,1)) ;
          for z=nz-1:-1:1
          rho_t(:,z) = rho_t(:,z) + z*0.0000001 ;
          end
          var_t = coeft(t,3).*vars_bry_west(:,:,coeft(t,2),v) + (1-coeft(t,3)).*vars_bry_west(:,:,coeft(t,1),v) ;
          var_interp = var_t.*NaN ;
          for i=1:size(rho_t,1)
%              var_int(i,:,t) = interp1(squeeze(rho_t(i,:)),squeeze(var_t(i,:)), ...
%                                       squeeze(dens_bry(i,:,t)),'nearest','extrap');
