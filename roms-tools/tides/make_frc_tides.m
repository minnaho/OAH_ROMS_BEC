...
clear all
   ...
   gname = '/data/project3/pdamien/ROMS_pdamien/config/SWELL/uswc1200_grd.nc' ;
   fname = '/data/project3/pdamien/ROMS_pdamien/config/SWELL/uswc1200_tidestpxo9_19950101.nc' ;
...
% tlonr = tlonr-180 ; % atl grid has the zero meridien %
 ...
 tlonr = ncread(tname,'lon_r',[i0 j0],[tnx 1]); %tlonr = tlonr-180 ;
 tlonu = ncread(tname,'lon_u',[i0 j0],[tnx 1]); %tlonu = tlonu-180 ;
 tlonv = ncread(tname,'lon_v',[i0 j0],[tnx 1]); %tlonv = tlonv-180 ;
 tlatr = ncread(tname,'lat_r',[i0 j0],[1 tny]);
 tlatu = ncread(tname,'lat_u',[i0 j0],[1 tny]);
 tlatv = ncread(tname,'lat_v',[i0 j0],[1 tny]);

 tlon2d = ncread(tname,'lon_r',[i0 j0],[tnx tny]); %tlon2d = tlon2d -180 ; 
