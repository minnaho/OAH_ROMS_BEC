...
clear all
   ...
   tname = '../../Tides_data/tpxo9.v2a.nc';

   gname = '/data/project7/pdamien/ROMS_outputs/ATLMED12KM/FORCINGS/atlmed_grd.nc' ;
   fname = '/data/project7/pdamien/ROMS_outputs/ATLMED12KM/FORCINGS/atlmed_tidestpxo9_19950101.nc' ;
...
% lon(lon<0) = lon(lon<0)+360;  % tpxo is between 0 and 360;
 ...
 tlonr = [ncread(tname,'lon_r',[1081 1],[inf 1])'-360 ncread(tname,'lon_r',[1 1],[1080 1])']';
...
% tlonr = ncread(tname,'lon_r',[i0 j0],[tnx 1]); 
 tlonr = [ncread(tname,'lon_r',[1081 j0],[inf 1])'-360 ncread(tname,'lon_r',[1 j0],[1080 1])']';
 tlonr = tlonr(i0:i0+tnx-1) ; 
% tlonu = ncread(tname,'lon_u',[i0 j0],[tnx 1]);
 tlonu = [ncread(tname,'lon_u',[1081 j0],[inf 1])'-360 ncread(tname,'lon_u',[1 j0],[1080 1])']';
 tlonu = tlonu(i0:i0+tnx-1) ;
% tlonv = ncread(tname,'lon_v',[i0 j0],[tnx 1]); 
 tlonv = [ncread(tname,'lon_v',[1081 j0],[inf 1])'-360 ncread(tname,'lon_v',[1 j0],[1080 1])']';
 tlonv = tlonv(i0:i0+tnx-1) ;
...
% tlat2d = ncread(tname,'lat_r',[i0 j0],[tnx tny]);
 tlon2d = [ncread(tname,'lon_r',[1081 1],[inf inf])'-360 ncread(tname,'lon_r',[1 1],[1080 inf])']';
 tlat2d = [ncread(tname,'lat_r',[1081 1],[inf inf])'     ncread(tname,'lat_r',[1 1],[1080 inf])']';
 tlon2d=tlon2d(i0:i0+tnx-1,j0:j0+tny-1) ; 
 tlat2d=tlat2d(i0:i0+tnx-1,j0:j0+tny-1) ;
...
% thi = ncread(tname,'h_Im',[i0 j0 1],[tnx tny nc]);
 thr = permute([permute(ncread(tname,'h_Re',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'h_Re',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 thi = permute([permute(ncread(tname,'h_Im',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'h_Im',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 thr=thr(i0:i0+tnx-1,j0:j0+tny-1,:) ; 
 thi=thi(i0:i0+tnx-1,j0:j0+tny-1,:) ;
 thc = complex(thr,thi);
% tur = ncread(tname,'u_Re',[i0 j0 1],[tnx tny nc]);
% tui = ncread(tname,'u_Im',[i0 j0 1],[tnx tny nc]);
 tur = permute([permute(ncread(tname,'u_Re',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'u_Re',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tui = permute([permute(ncread(tname,'u_Im',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'u_Im',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tur=tur(i0:i0+tnx-1,j0:j0+tny-1,:) ;    
 tui=tui(i0:i0+tnx-1,j0:j0+tny-1,:) ; 
 tuc = complex(tur,tui);
% tvr = ncread(tname,'v_Re',[i0 j0 1],[tnx tny nc]);
% tvi = ncread(tname,'v_Im',[i0 j0 1],[tnx tny nc]);
 tvr = permute([permute(ncread(tname,'v_Re',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'v_Re',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tvi = permute([permute(ncread(tname,'v_Im',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'v_Im',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tvr=tvr(i0:i0+tnx-1,j0:j0+tny-1,:) ;
 tvi=tvi(i0:i0+tnx-1,j0:j0+tny-1,:) ;
 tvc = complex(tvr,tvi);
% tsr = ncread(tname,'sal_Re',[i0 j0 1],[tnx tny nc]);
% tsi = ncread(tname,'sal_Im',[i0 j0 1],[tnx tny nc]);
 tsr = permute([permute(ncread(tname,'sal_Re',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'sal_Re',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tsi = permute([permute(ncread(tname,'sal_Im',[1081 1 1], [inf inf nc]),[2 1 3]) ... 
                permute(ncread(tname,'sal_Im',   [1 1 1],[1080 inf nc]),[2 1 3])],[2 1 3]);
 tsr=tsr(i0:i0+tnx-1,j0:j0+tny-1,:) ;
 tsi=tsi(i0:i0+tnx-1,j0:j0+tny-1,:) ;
 tsc = 2.0*complex(tsr,tsi);           % Alan factor 2.0
