...
##################################
dataset_name = 'pCO2_avg_monthly.nc'
...
# make time variable anthpco2 (same as roms_dust.nc)
pco2_time = np.array([15.21875,45.65625,76.09375,106.5312,136.9688,167.4062,197.8438,228.2812,258.7188,289.1562,319.5938,350.0312])
...
data_interp = Dataset('L2_SCB_pCO2_monthly_climatology.nc','w')
...
data_interp.description = '12 time steps, 1 for each month'

time = data_interp.createDimension('time',None)
eta_d = data_interp.createDimension('eta_rho',eta_rho)
xi_d = data_interp.createDimension('xi_rho',xi_rho)

pco2_nc = data_interp.createVariable('anthpco2',np.float32,('time','eta_rho','xi_rho')) 
time_nc = data_interp.createVariable('anthpco2_time',np.float32,('time')) 

pco2_nc[:,:,:] = pco2_interp_arr
time_nc[:] = pco2_time
time_nc.units = 'days'
time_nc.long_name = 'anthpco2 time'
time_nc.cycle_length = 365.25
