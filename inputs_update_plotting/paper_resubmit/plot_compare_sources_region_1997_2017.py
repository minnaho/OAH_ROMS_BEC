...
r_din_ssd = np.nanmean(np.nansum(np.array(r_river_di[0][0]),axis=0).reshape(ry0,12),axis=1)
r_din_nsd = np.nanmean(np.nansum(np.array(r_river_di[1][0]),axis=0).reshape(ry0,12),axis=1)
r_din_occ = np.nanmean(np.nansum(np.array(r_river_di[2][0]),axis=0).reshape(ry0,12),axis=1)
r_din_spp = np.nanmean(np.nansum(np.array(r_river_di[3][0]),axis=0).reshape(ry0,12),axis=1)
r_din_smm = np.nanmean(np.nansum(np.array(r_river_di[4][0]),axis=0).reshape(ry0,12),axis=1)
r_din_ven = np.nanmean(np.nansum(np.array(r_river_di[5][0]),axis=0).reshape(ry0,12),axis=1)
r_din_sbb = np.nanmean(np.nansum(np.array(r_river_di[6][0]),axis=0).reshape(ry0,12),axis=1)
...
# mexican input calculation
r_din_ssd_yr = np.nanmean(np.nansum(np.array(r_river_di[0][0]),axis=0).reshape(ry0,12),axis=0) 
...
# mexican inputs
p_major_din_ssd_yr = np.nanmean(np.nansum(np.array(p_major_di[0][0]),axis=0).reshape(ry0,12),axis=0) 
...
inl_tnn = inland_tnn[:7]
inl_tnn[3] = r_yr_nobight_tn[3]*.95

################################
# numbers in text calculations
################################
# don't include inland b/c should be accounted in rivers
# oc, sp, sm TN vs entire bight
ocspsm_tn = np.nansum(p_yr_tn[2:5]+r_yr_tn[2:5]+nat_cur_tnn[2:5])/(p_yr_tn[7]+r_yr_tn[7]+np.nansum(nnt_tnn))
print('total oc, sp, sm TN vs entire bight',ocspsm_tn)

# oc, sp, sm TN inland PS vs ocean PS vs total
ocspsm_in = np.nansum(inland_tnn[2]+r_yr_tn[3]*.95+inland_tnn[4])/np.nansum((p_yr_tn[2:5]+r_yr_tn[2:5]+nat_cur_tnn[2:5]))
print('inland oc, sp, sm TN vs total in region',ocspsm_in)

# ocean outfalls % TN in oc,sp,sm compared to total in that region
ocspsm_oo = np.nansum(p_yr_tn[2:5])/np.nansum((p_yr_tn[2:5]+r_yr_tn[2:5]+nat_cur_tnn[2:5]))
print('outfalls oc, sp, sm TN vs total in region',ocspsm_oo)

# oc, sp, sm TN NPS vs total in SCB
ocspsm_np = np.nansum(nps_tnn[2:5])/np.nansum((p_yr_tn[2:5]+r_yr_tn[2:5]+nat_cur_tnn[2:5]))
print('NPS oc, sp, sm TN vs total in region',ocspsm_np)

# ssd TN PS vs others
ssd_ps = np.nansum(p_yr_tn[0])/np.nansum((p_yr_tn[0]+r_yr_tn[0]+nat_cur_tnn[0]))
print('ssd PS TN vs total in ssd',ssd_ps)

# NPS ventura and SB compared to others
ven_np = np.nansum(nps_tnn[5])/np.nansum((p_yr_tn[5]+r_yr_tn[5]+nat_cur_tnn[5]))
print('ven NPS TN vs total in ven',ven_np)
ven_in = inl_tnn[5]/(p_yr_tn[5]+r_yr_tn[5]+nat_cur_tnn[5])
print('ven inland TN vs total in ven',ven_in)
ven_ps = p_yr_tn[5]/(p_yr_tn[5]+r_yr_tn[5]+nat_cur_tnn[5])
print('ven outfall PS TN vs total in ven',ven_ps)
ssb_np = nps_tnn[6]/np.nansum((p_yr_tn[6]+r_yr_tn[6]+nat_cur_tnn[6]))
print('sbb NPS TN vs total in sbb',ssb_np)
ssb_ps = p_yr_tn[6]/np.nansum((p_yr_tn[6]+r_yr_tn[6]+nat_cur_tnn[6]))
print('sbb outfall PS TN vs total in sbb',ssb_ps)
nsd_ps = p_yr_tn[1]/np.nansum((p_yr_tn[1]+r_yr_tn[1]+nat_cur_tnn[1]))
print('nsd outfall PS TN vs total in nsd',nsd_ps)

nsd_ps = np.nansum(nps_tnn[1])/np.nansum((p_yr_tn[1]+r_yr_tn[1]+nat_cur_tnn[1]))
print('nsd NPS TN vs total in nsd',nsd_ps)

inl_ps = inland_tnn[7]/(nps_tnn[7]+inland_tnn[7]+nat_cur_tnn[7])
print('inland TN vs riverine TN loads',inl_ps)

nps_ps = nps_tnn[7]/(nps_tnn[7]+inland_tnn[7]+nat_cur_tnn[7])
print('NPS TN vs riverine TN loads',nps_ps)


# peak flows major POTW and year
np.where(np.nansum(major_flo,axis=1)==np.nanmax(np.nansum(major_flo,axis=1)))

###################################################
# DIN per region comparing 1997-2000 and 2013-2017
###################################################
p_din_1997_2000_ssd = np.nanmean(p_din_ssd[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_nsd = np.nanmean(p_din_nsd[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_occ = np.nanmean(p_din_occ[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_spp = np.nanmean(p_din_spp[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_smm = np.nanmean(p_din_smm[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_ven = np.nanmean(p_din_ven[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_1997_2000_sbb = np.nanmean(p_din_sbb[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
               
p_din_2013_2017_ssd = np.nanmean(p_din_ssd[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_nsd = np.nanmean(p_din_nsd[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_occ = np.nanmean(p_din_occ[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_spp = np.nanmean(p_din_spp[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_smm = np.nanmean(p_din_smm[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_ven = np.nanmean(p_din_ven[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
p_din_2013_2017_sbb = np.nanmean(p_din_sbb[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol

isl_ind = [1,12]

p_din_1997_2000_isl = np.nanmean(np.nansum((minor_flo[:12*4,isl_ind]*minor_tn[:12*4,isl_ind]),axis=1)*s_to_d*g_N*g_to_kg*mmol_to_mol)
p_din_2013_2017_isl = np.nanmean(np.nansum((minor_flo[minor_flo.shape[0]-(12*5):,isl_ind]*minor_tn[minor_flo.shape[0]-(12*5):,isl_ind]),axis=1)*s_to_d*g_N*g_to_kg*mmol_to_mol)

p_din_1997_2000_scb = p_din_1997_2000_ssd+ p_din_1997_2000_nsd+ p_din_1997_2000_occ+ p_din_1997_2000_spp+ p_din_1997_2000_smm+ p_din_1997_2000_ven+ p_din_1997_2000_sbb+ p_din_1997_2000_isl
p_din_2013_2017_scb = p_din_2013_2017_ssd+ p_din_2013_2017_nsd+ p_din_2013_2017_occ+ p_din_2013_2017_spp+ p_din_2013_2017_smm+ p_din_2013_2017_ven+ p_din_2013_2017_sbb+ p_din_2013_2017_isl

               
r_din_1997_2000_ssd = np.nanmean(r_din_ssd[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_nsd = np.nanmean(r_din_nsd[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_occ = np.nanmean(r_din_occ[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_spp = np.nanmean(r_din_spp[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_smm = np.nanmean(r_din_smm[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_ven = np.nanmean(r_din_ven[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_1997_2000_sbb = np.nanmean(r_din_sbb[:4])*s_to_d*g_N*g_to_kg*mmol_to_mol
               
r_din_2013_2017_ssd = np.nanmean(r_din_ssd[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_nsd = np.nanmean(r_din_nsd[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_occ = np.nanmean(r_din_occ[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_spp = np.nanmean(r_din_spp[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_smm = np.nanmean(r_din_smm[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_ven = np.nanmean(r_din_ven[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol
r_din_2013_2017_sbb = np.nanmean(r_din_sbb[p_din_ssd.shape[0]-5:])*s_to_d*g_N*g_to_kg*mmol_to_mol

r_din_1997_2000_scb = r_din_1997_2000_ssd +r_din_1997_2000_nsd +r_din_1997_2000_occ +r_din_1997_2000_spp +r_din_1997_2000_smm +r_din_1997_2000_ven +r_din_1997_2000_sbb
r_din_2013_2017_scb = r_din_2013_2017_ssd +r_din_2013_2017_nsd +r_din_2013_2017_occ +r_din_2013_2017_spp +r_din_2013_2017_smm +r_din_2013_2017_ven +r_din_2013_2017_sbb

print('\n')
print('ssd DIN kg/d POTW 1997-2000:',p_din_1997_2000_ssd)
print('nsd DIN kg/d POTW 1997-2000:',p_din_1997_2000_nsd)
print('occ DIN kg/d POTW 1997-2000:',p_din_1997_2000_occ)
print('spp DIN kg/d POTW 1997-2000:',p_din_1997_2000_spp)
print('smm DIN kg/d POTW 1997-2000:',p_din_1997_2000_smm)
print('ven DIN kg/d POTW 1997-2000:',p_din_1997_2000_ven)
print('sbb DIN kg/d POTW 1997-2000:',p_din_1997_2000_sbb)
print('SCB DIN kg/d POTW 1997-2000:',p_din_1997_2000_scb)
print('\n')

print('ssd DIN kg/d POTW 2013-2017:',p_din_2013_2017_ssd)
print('nsd DIN kg/d POTW 2013-2017:',p_din_2013_2017_nsd)
print('occ DIN kg/d POTW 2013-2017:',p_din_2013_2017_occ)
print('spp DIN kg/d POTW 2013-2017:',p_din_2013_2017_spp)
print('smm DIN kg/d POTW 2013-2017:',p_din_2013_2017_smm)
print('ven DIN kg/d POTW 2013-2017:',p_din_2013_2017_ven)
print('sbb DIN kg/d POTW 2013-2017:',p_din_2013_2017_sbb)
print('SCB DIN kg/d POTW 2013-2017:',p_din_2013_2017_scb)
print('\n')

print('ssd DIN kg/d POTW % change:',(p_din_2013_2017_ssd-p_din_1997_2000_ssd)/p_din_1997_2000_ssd)
print('nsd DIN kg/d POTW % change:',(p_din_2013_2017_nsd-p_din_1997_2000_nsd)/p_din_1997_2000_nsd)
print('occ DIN kg/d POTW % change:',(p_din_2013_2017_occ-p_din_1997_2000_occ)/p_din_1997_2000_occ)
print('spp DIN kg/d POTW % change:',(p_din_2013_2017_spp-p_din_1997_2000_spp)/p_din_1997_2000_spp)
print('smm DIN kg/d POTW % change:',(p_din_2013_2017_smm-p_din_1997_2000_smm)/p_din_1997_2000_smm)
print('ven DIN kg/d POTW % change:',(p_din_2013_2017_ven-p_din_1997_2000_ven)/p_din_1997_2000_ven)
print('sbb DIN kg/d POTW % change:',(p_din_2013_2017_sbb-p_din_1997_2000_sbb)/p_din_1997_2000_sbb)
print('SCB DIN kg/d POTW % change:',(p_din_2013_2017_scb-p_din_1997_2000_scb)/p_din_1997_2000_scb)
print('\n')

print('ssd DIN kg/d river 1997-2000:',r_din_1997_2000_ssd)
print('nsd DIN kg/d river 1997-2000:',r_din_1997_2000_nsd)
print('occ DIN kg/d river 1997-2000:',r_din_1997_2000_occ)
print('spp DIN kg/d river 1997-2000:',r_din_1997_2000_spp)
print('smm DIN kg/d river 1997-2000:',r_din_1997_2000_smm)
print('ven DIN kg/d river 1997-2000:',r_din_1997_2000_ven)
print('sbb DIN kg/d river 1997-2000:',r_din_1997_2000_sbb)
print('SCB DIN kg/d river 1997-2000:',r_din_1997_2000_scb)
print('\n')

print('ssd DIN kg/d river 2013-2017:',r_din_2013_2017_ssd)
print('nsd DIN kg/d river 2013-2017:',r_din_2013_2017_nsd)
print('occ DIN kg/d river 2013-2017:',r_din_2013_2017_occ)
print('spp DIN kg/d river 2013-2017:',r_din_2013_2017_spp)
print('smm DIN kg/d river 2013-2017:',r_din_2013_2017_smm)
print('ven DIN kg/d river 2013-2017:',r_din_2013_2017_ven)
print('sbb DIN kg/d river 2013-2017:',r_din_2013_2017_sbb)
print('SCB DIN kg/d river 2013-2017:',r_din_2013_2017_scb)
print('\n')

print('ssd DIN kg/d river % change:',(r_din_2013_2017_ssd-r_din_1997_2000_ssd)/r_din_1997_2000_ssd)
print('nsd DIN kg/d river % change:',(r_din_2013_2017_nsd-r_din_1997_2000_nsd)/r_din_1997_2000_nsd)
print('occ DIN kg/d river % change:',(r_din_2013_2017_occ-r_din_1997_2000_occ)/r_din_1997_2000_occ)
print('spp DIN kg/d river % change:',(r_din_2013_2017_spp-r_din_1997_2000_spp)/r_din_1997_2000_spp)
print('smm DIN kg/d river % change:',(r_din_2013_2017_smm-r_din_1997_2000_smm)/r_din_1997_2000_smm)
print('ven DIN kg/d river % change:',(r_din_2013_2017_ven-r_din_1997_2000_ven)/r_din_1997_2000_ven)
print('sbb DIN kg/d river % change:',(r_din_2013_2017_sbb-r_din_1997_2000_sbb)/r_din_1997_2000_sbb)
print('SCB DIN kg/d river % change:',(r_din_2013_2017_scb-r_din_1997_2000_scb)/r_din_1997_2000_scb)
print('\n')

# SBIWTP and TJ river vs total DIN in SSD
# unconverted, mmol/s
ssd_din = np.nansum(p_major_din_ssd_yr+p_minor_tnn_ssd+r_din_ssd_yr)
sbiwtp_din = np.nanmean(np.nansum(minor_tn[:,15].reshape(21,12),axis=0))
tj_din = np.nanmean(np.nansum(river_din[:,68].reshape(21,12),axis=0))
