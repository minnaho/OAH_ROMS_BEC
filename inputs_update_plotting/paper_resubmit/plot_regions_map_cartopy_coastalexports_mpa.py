...
#fig,ax = plt.subplots(1,1,figsize=[fig_w,fig_h],subplot_kw=dict(projection=ccrs.EckertIV()))
# plot region masks
...
# MPA
#shpfile = '/data/project1/data/MPAs/NOAA_MPAI/NOAA_MPAI.shp'
shpfile = '/data/project1/data/MPAs/NOAA_MPAI_2020_IUCN_gdb/mpa.shp'
#mpashp = cpf.ShapelyFeature(shpreader.Reader(shpfile).geometries(),ccrs.PlateCarree(),edgecolor='black',facecolor='None')
mpashp = cpf.ShapelyFeature(shpreader.Reader(shpfile).geometries(),ccrs.EckertIV(),edgecolor='black',facecolor='None')
ax.add_feature(mpashp,transform=ccrs.PlateCarree())
...
# region names
ax.text(-120.1,34.24,'SB',fontsize=axis_tick_size)
ax.text(-119.3,33.93,'V',fontsize=axis_tick_size)
ax.text(-118.7,33.8,'SM',fontsize=axis_tick_size)
ax.text(-118.4,33.51,'SP',fontsize=axis_tick_size)
ax.text(-118,33.28,'OC',fontsize=axis_tick_size)
ax.text(-117.6,32.89,'NSD',fontsize=axis_tick_size)
ax.text(-117.55,32.55,'SSD',fontsize=axis_tick_size)
...
#plt.savefig('figs/inputs_updated_map.pdf',bbox_inches='tight')
