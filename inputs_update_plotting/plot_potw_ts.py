...
minor_path = '/data/project1/minnaho/potw_outfall_data/updated_2013_2017/minor_potw_data/minor_potw_1997_2017.nc'
fig_path = './figs/'
...
minor_time_dt = num2date(np.array(minor_nc.variables['time']),minor_nc.variables['time'].units,only_use_cftime_datetimes=False)
...
for p_i in range(len(major_names)):
    axes.flat[0].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[1].plot(major_time_dt[:iend_major],major_tn[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    if p_i == 3:   
        axes.flat[2].plot(major_time_dt[plwtp_st:iend_major],major_po4[plwtp_st:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    else:
        axes.flat[2].plot(major_time_dt[:iend_major],major_po4[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
...
for p_i in range(len(major_names)):
    ...
    if p_i == 3:   
        ...
        axes.flat[2].plot(major_time_dt[:iend_major],major_po4[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[3].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_tn[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[4].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_po4[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
axes.flat[0].set_ybound(lower=0)
axes.flat[1].set_ybound(lower=0)
axes.flat[2].set_ybound(lower=0)
...
for p_i in range(len(major_names)):
    ...
    axes.flat[1].plot(major_time_dt[:iend_major],major_tn[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    if p_i == 3:   
        axes.flat[2].plot(major_time_dt[plwtp_st:iend_major],major_tp[plwtp_st:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    else:
        axes.flat[2].plot(major_time_dt[:iend_major],major_tp[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[3].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_tn[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[4].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_tp[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
axes.flat[0].set_ybound(lower=0)
axes.flat[1].set_ybound(lower=0)
axes.flat[2].set_ybound(lower=0)
axes.flat[3].set_ybound(lower=0)
...
for p_i in range(len(major_names)):
    axes.flat[0].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[1].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_tn[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[2].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_tp[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[3].plot(major_time_dt[:iend_major],major_flo[:iend_major,p_i]*major_toc[:iend_major,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
axes.flat[0].set_ybound(lower=0)
axes.flat[1].set_ybound(lower=0)
axes.flat[2].set_ybound(lower=0)
axes.flat[3].set_ybound(lower=0)
...
for m_i in range(major_flo.shape[1]):
    a.append(major_flo[:,m_i]*major_tn[:,m_i])

major_potw_fluxalln = np.nansum(np.array(a),axis=0)

b = []
for m_i in range(major_flo.shape[1]):
    b.append(major_flo[:,m_i]*major_tp[:,m_i])

c = []
for m_i in range(major_flo.shape[1]):
    c.append(major_flo[:,m_i]*major_toc[:,m_i])
...
fig,axes = plt.subplots(3,1,sharex=True,figsize=[figw,figh])
axes.flat[0].plot(major_time_dt[:iend_major-1],np.nansum(major_flo[:iend_major-1],axis=1),linewidth=lw)
...
fig,axes = plt.subplots(3,1,sharex=True,figsize=[figw,figh])
axes.flat[0].plot(major_time_dt[st_in:iend_major-1],np.nansum(major_flo[st_in:iend_major-1],axis=1),linewidth=lw)
...
plt.ion()
# din only
savename_major_flux = fig_path+'major_potw_ts_din_allyears_2col.png'
ubndfl = 20
ubndmg = 60
ubndkg = 60000

fig,axes = plt.subplots(4,2,sharex=True,figsize=[figw+5,figh])
axes.flat[0].plot(major_time_dt[:iend_major],major_flo[:iend_major,0],linestyle=major_linesty[0],label=major_names[0],linewidth=lw,color='C0')
axes.flat[0].set_ybound(lower=0,upper=ubndfl)
axes.flat[0].set_ylabel(major_names[0],fontsize=axis_font)

axes.flat[2].plot(major_time_dt[:iend_major],major_flo[:iend_major,1],linestyle=major_linesty[1],label=major_names[1],linewidth=lw,color='C1')
axes.flat[2].set_ybound(lower=0,upper=ubndfl)
axes.flat[2].set_ylabel(major_names[1],fontsize=axis_font)

axes.flat[4].plot(major_time_dt[:iend_major],major_flo[:iend_major,2],linestyle=major_linesty[2],label=major_names[2],linewidth=lw,color='C2')
axes.flat[4].set_ybound(lower=0,upper=ubndfl)
axes.flat[4].set_ylabel(major_names[2],fontsize=axis_font)

axes.flat[6].plot(major_time_dt[:iend_major],major_flo[:iend_major,3],linestyle=major_linesty[3],label=major_names[3],linewidth=lw,color='C3')
axes.flat[6].set_ybound(lower=0,upper=ubndfl)
axes.flat[6].set_ylabel(major_names[3],fontsize=axis_font)

axes.flat[1].plot(major_time_dt[:iend_major],major_flo[:iend_major,0]*major_din[:iend_major,0]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[0],label=major_names[0],linewidth=lw,color='C0')
axes.flat[1].set_ybound(lower=0,upper=ubndkg)

axes.flat[3].plot(major_time_dt[:iend_major],major_flo[:iend_major,1]*major_din[:iend_major,1]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[1],label=major_names[1],linewidth=lw,color='C1')
axes.flat[3].set_ybound(lower=0,upper=ubndkg)

axes.flat[5].plot(major_time_dt[:iend_major],major_flo[:iend_major,2]*major_din[:iend_major,2]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[2],label=major_names[2],linewidth=lw,color='C2')
axes.flat[5].set_ybound(lower=0,upper=ubndkg)

axes.flat[7].plot(major_time_dt[:iend_major],major_flo[:iend_major,3]*major_din[:iend_major,3]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[3],label=major_names[3],linewidth=lw,color='C3')
axes.flat[7].set_ybound(lower=0,upper=ubndkg)

axes.flat[0].set_xlabel('Volume Flux m$^3$ s$^{-1}$',fontsize=axis_font)
axes.flat[0].xaxis.set_label_position('top')
axes.flat[1].set_xlabel('DIN Flux kg d$^{-1}$',fontsize=axis_font)
axes.flat[1].xaxis.set_label_position('top')
for ax_i in range(8):
...
for m_i in range(minor_flo.shape[1]):
    a.append(minor_flo[:,m_i]*minor_din[:,m_i])

minor_din_flux = np.nansum(np.array(a),axis=0)

# zoom to 1997-2000 and 2013-2017
ind_1997 = 9497
ind_2000 = 10957
...
mi_ind_2000 = 1460 
mi_ind_2013 = 5844
mi_ind_2017 = minor_time_dt.shape[0]
savename_major_flux = fig_path+'major_potw_ts_din_split.png'

fig,axes = plt.subplots(3,2,figsize=[figw+5,figh+5])
for p_i in range(len(major_names)):
    axes.flat[0].plot(major_time_dt[ind_1997:ind_2000],major_flo[ind_1997:ind_2000,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[1].plot(major_time_dt[ind_2013:ind_2017],major_flo[ind_2013:ind_2017,p_i],linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    # din mg/L
    axes.flat[2].plot(major_time_dt[ind_1997:ind_2000],major_din[ind_1997:ind_2000,p_i]*mmol_to_mol*g_N,linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[3].plot(major_time_dt[ind_2013:ind_2017],major_din[ind_2013:ind_2017,p_i]*mmol_to_mol*g_N,linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    # din kg/day
    axes.flat[4].plot(major_time_dt[ind_1997:ind_2000],major_flo[ind_1997:ind_2000,p_i]*major_din[ind_1997:ind_2000,p_i]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)
    axes.flat[5].plot(major_time_dt[ind_2013:ind_2017],major_flo[ind_2013:ind_2017,p_i]*major_din[ind_2013:ind_2017,p_i]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)

ubndfl = 20
ubndmg = 60
ubndkg = 60000
axes.flat[0].set_ybound(lower=0,upper=ubndfl)
axes.flat[1].set_ybound(lower=0,upper=ubndfl)
axes.flat[2].set_ybound(lower=0,upper=ubndmg)
axes.flat[3].set_ybound(lower=0,upper=ubndmg)
axes.flat[4].set_ybound(lower=0,upper=ubndkg)
axes.flat[5].set_ybound(lower=0,upper=ubndkg)
axes.flat[0].set_ylabel('Volume Flux\n m$^3$ s$^{-1}$',fontsize=axis_font)
axes.flat[2].set_ylabel('DIN\n mg L$^{-1}$',fontsize=axis_font)
axes.flat[4].set_ylabel('DIN Flux\n kg d$^{-1}$',fontsize=axis_font)
for ax_i in range(axes.shape[0]*axes.shape[1]):
    axes.flat[ax_i].tick_params(axis='both',which='major',labelsize=axis_tick_font)
    axes.flat[ax_i].xaxis.set_major_locator(matplotlib.dates.YearLocator())
    axes.flat[ax_i].xaxis.set_major_formatter(matplotlib.dates.DateFormatter("%Y"))

axes.flat[0].legend(loc='lower left',fontsize=axis_tick_font,bbox_to_anchor=[0,1.02,2.2,.102],ncol=4,mode='expand',borderaxespad=0.,handlelength=3)
#loc = mtick.MultipleLocator(base=50000) 
#axes.flat[3].yaxis.set_major_locator(loc)
fig.savefig(savename_major_flux,bbox_inches='tight')

# din only
savename_major_flux = fig_path+'major_potw_ts_din_and_minors.png'


fig,axes = plt.subplots(1,1,sharex=True,figsize=[figw+5,figh])
for p_i in range(len(major_names)):
    # din kg/day
    axes.plot(major_time_dt[ind_1997:iend_major],major_flo[ind_1997:iend_major,p_i]*major_din[ind_1997:iend_major,p_i]*s_to_d*mmol_to_mol*g_to_kg*g_N,linestyle=major_linesty[p_i],label=major_names[p_i],linewidth=lw)

axes.plot(minor_time_dt,minor_din_flux*s_to_d*mmol_to_mol*g_to_kg*g_N,label='Summed Minor POTWs',color='orchid',linewidth=lw)
axes.set_ybound(lower=0)
axes.set_ylabel('DIN Flux\n kg d$^{-1}$',fontsize=axis_font)
axes.tick_params(axis='both',which='major',labelsize=axis_tick_font)
axes.legend(loc='lower left',fontsize=axis_tick_font,bbox_to_anchor=[0,1.02,1,.102],ncol=4,mode='expand',borderaxespad=0.,handlelength=3)
#loc = mtick.MultipleLocator(base=50000) 
#axes.flat[3].yaxis.set_major_locator(loc)
fig.savefig(savename_major_flux,bbox_inches='tight')
