...
# bulk_forcing
tide     = 'roms_tide_L1.nc'
atm_wind = 'ussw1_wnd_'
atm_tra  = 'ussw1_tra_'
atm_rad  = 'ussw1_rad_'
atm_prec = 'ussw1_prec_'
frc      = 'usw1_frc.nc'
dust     = 'ussw1_dust.nc'
...
# DON'T SAVE THESE FOR SUB DAILY OUTPUT WHEN EXISTING MODEL OUTPUT
...
for y in range(start_year,end_year+1):
    ...
    for m in range(s_m,e_m): 
        ...
        # save every 2 hours
        f.write('             '+LDEFHHIS+'    '+str(int(daytimes/12))+'   '+NRPFHIS+'\n')
        f.write('              '+'AVG_'+year_month+'/'+HISFILE+'\n\n')

        f.write('averages: NTSAVG, NAVG, NRPFAVG / filename\n')
        #f.write('            '+NTSAVG+'   '+str(ntimes)+'    '+NRPFAVG+'\n')
        # don't save
        f.write('            '+NTSAVG+'   '+nwrt_bd+'    '+NRPFAVG+'\n')
        f.write('              '+'AVG_'+year_month+'/'+AVGFILE+'\n\n')

        # check param.h for 1:NT (first 5 are zeta, ubar, vbar, u, v)
        # next 2 are temp and salt, then rest are bgc variables
        f.write('primary_history_fields: zeta U,VBAR  U,V   wrtT(1:NT)\n')
        f.write('            5*T 6*T 1*F 3*T 1*F 1*T 2*F 1*T 1*F 1*T 3*F 1*T 33*F\n\n')
        ...
        f.write('                      '+newfile+'      '+nwrt_bd+'     '+nrpfhis+'\n')
        ...
        f.write('            '+newfile+'   '+NTSAVG+'   '+nwrt_bd+'    '+NRPFAVG+'\n')
        ...
        f.write('                       '+newfile_bda+'    '+ntsavg_bda+'       '+nwrt_bd+'   '+nrpfavg_bda+'\n')
