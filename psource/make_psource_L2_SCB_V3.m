...
psrc_fname='psource_L2_scbV2.nc';
...
% HTP : 1 --> 20 (2 MAJORS) 
Qbar(:,1) = flow(:,1).*(8/36) ; % HTP1 (N)
Qbar(:,2) = flow(:,1).*(8/36) ; % HTP2 (S)
for i=3:20
        Qbar(:,i) = flow(:,1).*(2/36) ; % HTP Small flow around the main diffusers
end
for i=3:20
        Qbar(:,i) = flow(:,1).*(2/36) ; % HTP Small flow around the main diffusers
end
for i=3:20
	Qbar(:,i) = flow(:,1).*(0.5/36) ; % HTP Small flow around the main diffusers
end
...
% OCSD : 45 --> 53 (1 MAJOR)
Qbar(:,45) = flow(:,3).*(16/36) ; % OCSD --> 100% 
for i=46:53
        Qbar(:,i) = flow(:,3).*(4/36) ; % HTP Small flow around the main diffusers 2 and 3 (N)
end
for i=46:53
...
Qbar(:,54) = flow(:,1).*(8/36) ; % HTP1 (N)
Qbar(:,55) = flow(:,1).*(8/36) ; % HTP2 (S)
for i=56:71
        Qbar(:,i) = flow(:,1).*(2/36) ; % HTP Small flow around the main diffusers
end
for i=56:71
