...
psrc_fname='psource_L2_scbV7_CONC.nc';
...
% HTP : 1 --> 20 (2 MAJORS)
%Qbar(:,1) = flow(:,1).*(6/36) ; % HTP1 (N)
%Qbar(:,2) = flow(:,1).*(6/36) ; % HTP2 (S)

Qbar(:,1) = flow(:,1).*(1/4) ; % HTP1 (N)
Qbar(:,2) = flow(:,1).*(1/4) ; % HTP2 (S)

Qbar(:,3) = flow(:,1).*(1/4) ; % HTP1 (N)
Qbar(:,4) = flow(:,1).*(1/4) ; % HTP2 (S)

% JWPCP : 21 --> 44 (3 MAJORS: 1S AND 2N)
Qbar(:,5) = flow(:,2).*.65.*(1/3) ; % JWPCP1 --> 65% of total (S)
Qbar(:,6) = flow(:,2).*.65.*(1/3) ; % JWPCP2 --> 17.5% of total (N)
Qbar(:,7) = flow(:,2).*.65.*(1/3) ; % JWPCP3 __> 17.5% of total (N)

Qbar(:,8) = flow(:,2).*.35.*(1/2) ; % JWPCP3 __> 17.5% of total (N)
Qbar(:,9) = flow(:,2).*.35.*(1/2) ; % JWPCP3 __> 17.5% of total (N)
%Qbar(:,12) = flow(:,2).*.35.*(3/9) ; % JWPCP3 __> 17.5% of total (N)

% OCSD : 45 --> 53 (1 MAJOR)
Qbar(:,10) = flow(:,3).*(1/2) ; % OCSD --> 100%
%Qbar(:,14) = flow(:,3).*(6/18) ; % OCSD --> 100%
Qbar(:,11) = flow(:,3).*(1/2) ; % OCSD --> 100%

% PLWTP : 54 --> 71 (2 MAJORS)
Qbar(:,12) = flow(:,1).*(1/4) ; % PLWTP1 (N)
Qbar(:,13) = flow(:,1).*(1/4) ; % PLWTP2 (S)
Qbar(:,14) = flow(:,1).*(1/4) ; % PLWTP1 (N)
Qbar(:,15) = flow(:,1).*(1/4) ; % PLWTP2 (S)
...
if Riv==1
%% change Drsc
list = [86 88 91 92 93 94 95 104 106 113 114 115 116 120 121 122 127 128 130 132 135 136 137 145 146 148 150 154 157 160]-52-4 ;
for k=1:108-4
test = find (k==list) ;
if test>=1
Dsrc(k) = 1 ;
