...
%time = ( ptime + datenum(1997,1,1)  - datenum(1994,1,1) +1 );
test_t = find(time>0); datestart = test_t(1) ;
test_t = find(ptime>datenum(1997,1,1)) ;
...
%% JWPCP
for i= 21:44
        db(1,i) = 35 ; % above the bottom in meters (McLaughlin et al in prep)
        NO3(:,i) = nitrate(:,2);
        NH4(:,i) = ammonium(:,2);
        NO2(:,i) = nitrite(:,2);
        PO4(:,i) = phosphate(:,2);
...
%% PLWTP
for i=54:71
	db(1,i) = 35 ;% above the bottom in meters
        NO3(:,i) = nitrate(:,4);
        NH4(:,i) = ammonium(:,4);
        NO2(:,i) = nitrite(:,4);
        PO4(:,i) = phosphate(:,4);
...
%time = ( ptime + datenum(1997,1,1)  - datenum(1994,1,1) +1 ).*86400;
time = ( ptime + datenum(1970,1,1)  - datenum(1994,1,1) +1 );
test_t = find(time>0);
% remve suspecious data
test_t = test_t(1:180) ;
time_l = time(test_t) ;
...
%% DEFINE THE TIME
ptime = double(ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','time')); %
%time = ( ptime + datenum(1990,1,1)  - datenum(1994,1,1) +1 ).*86400;
time = ( ptime + datenum(1990,1,1)  - datenum(1994,1,1) +1 ) ;
test_t = find(time>0);
time_l = time(test_t) ;
test = find(psrc_time<time_l(1)) ;
if isempty(test)
...
netcdf.putVar(ncid,psr_time,psrc_time);
% Qbar
