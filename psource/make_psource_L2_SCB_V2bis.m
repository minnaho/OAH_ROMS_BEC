...
psrc_fname='psource_L2_scbV5.nc';
...
% HTP : 1 --> 20 (2 MAJORS) 
% main
Qbar(:,1) = flow(:,1).*(16/74) ; % HTP1 (N)
Qbar(:,2) = flow(:,1).*(16/74) ; % HTP2 (S)
% others
for i=3:20
        Qbar(:,i) = flow(:,1).*(1/74) ; % HTP Small flow around the main diffusers
end
% edges
Qbar(:,4) = flow(:,1).*(4/74) ; % 
Qbar(:,6) = flow(:,1).*(4/74) ; %
...
Qbar(:,21) = flow(:,2).*0.35.*(17/72) ; % JWPCP1 --> 65% of total (S)
Qbar(:,22) = flow(:,2).*0.35.*(17/72) ; % JWPCP2 --> 17.5% of total (N)
Qbar(:,23) = flow(:,2).*0.65.*(4/10) ; % JWPCP3 __> 17.5% of total (N)
% others for south
for i=24:32
        Qbar(:,i) = flow(:,2).*0.35.*(1/72) ; % HTP Small flow around the main diffusers 1 (S)
end
% edge for south
Qbar(:,25) = flow(:,2).*0.35.*(4/72) ;
Qbar(:,27) = flow(:,2).*0.35.*(4/72) ;
Qbar(:,30) = flow(:,2).*0.35.*(8/72) ;
Qbar(:,32) = flow(:,2).*0.35.*(8/72) ;
%Qbar(:,32) = flow(:,2).*0.35.*(4/72) ;
Qbar(:,34) = flow(:,2).*0.35.*(4/72) ;
Qbar(:,31) = flow(:,2).*0.35.*(4/72) ;
%Qbar(:,30) = flow(:,2).*0.35.*(4/72) ;

% others for north
for i=36:44
        Qbar(:,i) = flow(:,2).*.65.*(1/37) ; % HTP Small flow around the main diffusers 1 (S)
end
% edges for north
Qbar(:,37) = flow(:,2).*.65.*(4/37) ;
Qbar(:,40) = flow(:,2).*.65.*(4/37) ;
Qbar(:,42) = flow(:,2).*.65.*(4/37) ;
Qbar(:,39) = flow(:,2).*.65.*(4/37) ;
for i=33:44
...
% OCSD : 45 --> 53 (1 MAJOR)
% main
Qbar(:,45) = flow(:,3).*(9/36) ; % OCSD --> 100% 
% others
for i=46:53
        Qbar(:,i) = flow(:,3).*(1/36) ; % HTP Small flow around the main diffusers 2 and 3 (N)
end
% edges
Qbar(:,47) = flow(:,3).*(4/36) ;
Qbar(:,50) = flow(:,3).*(4/36) ;
...
% main
Qbar(:,54) = flow(:,4).*(16/72) ; % PLWTP1 (N)
Qbar(:,55) = flow(:,4).*(16/72) ; % PLWTP2 (S)
% others
for i=56:71
        Qbar(:,i) = flow(:,1).*(1/72) ; % PLWTP Small flow around the main diffusers
end
% edges
Qbar(:,57) = flow(:,4).*(4/72) ;
Qbar(:,60) = flow(:,4).*(4/72) ;
...
%% HTP
for i = 1:20
        db(1,i) = 10 ; % above the bottom in meters (Uchiyama et al 2014)
	NO3(:,i) = nitrate(:,1);
	NH4(:,i) = ammonium(:,1);
	NO2(:,i) = nitrite(:,1);
	PO4(:,i) = phosphate(:,1);
...
%% PLWTP
for i=54:71
	db(1,i) = 10 ;% above the bottom in meters
        NO3(:,i) = nitrate(:,4);
        NH4(:,i) = ammonium(:,4);
        NO2(:,i) = nitrite(:,4);
        PO4(:,i) = phosphate(:,4);
...
%	O2(datestart:dateend,i) = 0.*flow(:,i-psNOW)+(5.6/16*1000);
        O2(datestart:dateend,i) = 0.*flow(:,i-psNOW)+(2/16*1000);
...
pflow = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','flow'); % m3/s
pNO3 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','nitrate'); %
pNH4 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','ammonium'); %
pPO4 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','phosphate'); %
pTON = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','total_nitrogen'); %
pTOP = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','total_phosphorus'); %
palkalinity = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','alkalinity'); %
ptemp = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','temperature'); %
pDON = pTON - (pNH4+pNO3); pDON(pDON<0)=1e-10 ;
pDOP = pTOP - pPO4 ; pPO4(pPO4<0)=1e-10 ;

%% DEFINE THE TIME
ptime = ncread('/data/project1/minnaho/river_data/south_coast_rivers_10_years_monthly.nc','time'); %
...
disp('RIVERS 24 Y ... ')
%% OTHER RIVERS
pflow = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','flow'); % m3/s
pNO3 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','nitrate'); %
pNH4 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','ammonium'); %
pPO4 = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','phosphate'); %
pTON = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','total_nitrogen'); %
pTOP = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','total_phosphorus'); %
palkalinity = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','alkalinity'); %
ptemp = ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','temperature'); %
pDON = pTON - (pNH4+pNO3); pDON(pDON<0)=1e-10 ;
pDOP = pTOP - pPO4 ; pPO4(pPO4<0)=1e-10 ;

%% DEFINE THE TIME
ptime = double(ncread('/data/project1/minnaho/river_data/south_coast_rivers_24_years_monthly.nc','time')) + datenum(1990,1,1) ; %
...
%% change Drsc
list = [86 88 91 92 93 94 95 104 106 113 114 115 116 120 121 122 127 128 130 132 135 136 137 145 146 148 150 154 157 160] ;
for k=1:160
test = find (k==list) ;
if test>=1
Dsrc(k) = 1 ;
