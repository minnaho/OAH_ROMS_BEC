...
data_files = list(sorted(glob.glob(dataset_path+dataset_name+str(yr)+'*')))
...
for m_i in range(len(data_files)):
    print(str(m_i)+' of '+str(len(data_files)-1))
    data_arr = np.array(Dataset(data_files[m_i],'r').variables['npp'])
...
data_interp = Dataset('npp_'+alg_name_u+'_'+str(yr)+'.nc','w')
...
data_interp.description = 'monthly data'

time = data_interp.createDimension('time',len(data_files))
